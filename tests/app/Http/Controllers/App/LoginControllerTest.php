...
it('logs in a app user successfully', function (): void {
...
it('fails login with non existing app user email', function (): void {
    post(action([LoginController::class, 'login']), [
...
it('fails login when email is not verified', function (): void {
    $this->appUser->forceFill(['email_verified_at' => null])->save();
